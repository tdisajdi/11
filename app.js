...
[{"id":"job_...","type":"active","name":"...","icon":"...","rarity":"common","mpCost":10,"hpCost":0,"hpRestore":0,"req":{},"desc":"...","aiHint":"...","condition":null,"conditionDesc":null,"statBoost":{},"scenario":${JSON.stringify(scenarioId)},"jobRole":${JSON.stringify(roleName)}}, ...]`;
    ...
    try {
      ...
      const fb = [
        ...
        {id:"job_heavy",type:"active",name:"강타",icon:"💥",rarity:"uncommon",mpCost:15,hpCost:0,hpRestore:0,req:{str:20},desc:"강력한 일격으로 큰 피해를 입힌다.",aiHint:"강타를 날립니다.",condition:null,conditionDesc:null,statBoost:{},scenario:sid,jobRole:roleName},
        {id:"job_swift",type:"active",name:"신속 공격",icon:"💨",rarity:"common",mpCost:8,hpCost:0,hpRestore:0,req:{agi:15},desc:"빠른 속도로 공격한다.",aiHint:"빠르게 공격합니다.",condition:null,conditionDesc:null,statBoost:{},scenario:sid,jobRole:roleName},
        ...
        {id:"job_analyze",type:"active",name:"적 분석",icon:"🔍",rarity:"uncommon",mpCost:8,hpCost:0,hpRestore:0,req:{per:15},desc:"적의 약점을 파악한다.",aiHint:"예리하게 적을 분석합니다.",condition:null,conditionDesc:null,statBoost:{},scenario:sid,jobRole:roleName},
        {id:"job_power",type:"active",name:"힘의 급등",icon:"🔥",rarity:"rare",mpCost:25,hpCost:0,hpRestore:0,req:{str:25},desc:"순간적으로 모든 힘을 폭발시킨다.",aiHint:"힘이 폭발합니다.",condition:null,conditionDesc:null,statBoost:{},scenario:sid,jobRole:roleName},
        {id:"job_evade",type:"passive",name:"회피 본능",icon:"🌪️",rarity:"common",mpCost:0,hpCost:0,hpRestore:0,req:{agi:15},desc:"공격을 본능적으로 피한다. AGI +6.",aiHint:"날렵하게 피합니다.",condition:"in_combat",conditionDesc:"전투 중",statBoost:{agi:6},scenario:sid,jobRole:roleName},
        ...
        {id:"job_sense",type:"passive",name:"예리한 감각",icon:"👁️",rarity:"uncommon",mpCost:0,hpCost:0,hpRestore:0,req:{per:20},desc:"주변을 예리하게 감지한다. PER +8.",aiHint:"감각을 곤두세웁니다.",condition:"always",conditionDesc:"항시 발동",statBoost:{per:8},scenario:sid,jobRole:roleName},
        {id:"job_will",type:"passive",name:"강철 의지",icon:"💎",rarity:"rare",mpCost:0,hpCost:0,hpRestore:0,req:{wil:20},desc:"꺾이지 않는 의지. WIL +8.",aiHint:"의지를 드러냅니다.",condition:"always",conditionDesc:"항시 발동",statBoost:{wil:8},scenario:sid,jobRole:roleName},
        {id:"job_second",type:"event",name:"두 번째 바람",icon:"💪",rarity:"rare",mpCost:0,hpCost:0,hpRestore:25,req:{},desc:"위기 순간 숨겨진 힘이 솟구친다. HP 25 회복.",aiHint:"강하게 일어납니다.",condition:"hp_critical",conditionDesc:"HP 30% 이하",statBoost:{},scenario:sid,jobRole:roleName},
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      ...
      const LOCAL = {
        role: { medieval:["기사","마법사","도적","성직자","검사","팔라딘","레인저","연금술사","음유시인","궁수"], wuxia:["무사","검객","암살자","도사","의원","협객","신관","표국 무인","방랑자","속세 이탈자"], cyberpunk:["해커","용병","기업 스파이","사이버 의사","드론 조종사","정보 중개인","반란군 전사","네트러너","암거래상","AI 연구자"], apocalypse:["생존자","황무지 전사","의사","약탈자","정찰병","엔지니어","방랑자","폐허 탐험가","부족 주술사","요새 수비대"], mythology:["반신","영웅","신관","예언자","괴물 사냥꾼","운명의 전사","신들의 사자","음유시인","티탄 전사","오라클"], steampunk:["발명가","증기 기사","비행선 조종사","귀족 탐정","기계 의사","에테르 마법사","혁명가","암시장 딜러","산업 스파이","연금술사"] },
        background: ["평범한 삶을 살다가 운명이 바뀐 순간이 있었다. 그 사건이 지금의 나를 만들었다.","가족을 잃은 후 복수를 맹세했다. 하지만 시간이 지나며 그 의미가 흐려지고 있다.","누군가의 제자였으나 스승을 잃고 홀로 세상에 내던져졌다.","비밀 조직에 속해 있었다. 그들이 원하는 것과 내가 원하는 것이 달라지기 시작했다.","과거의 죄를 씻기 위해 방랑한다."],
      ...
      if (cur.key === "role") setSuggestions((LOCAL.role[sid] || ["전사","마법사","도적","성직자","탐정"]).sort(() => Math.random()-0.5).slice(0,5));
      else if (cur.key === "background") setSuggestions(LOCAL.background);
      else if (cur.key === "speechStyle") setSuggestions(LOCAL.speechStyle);
      else setSuggestions([]);
    }
    setSugLoading(false);
  };
